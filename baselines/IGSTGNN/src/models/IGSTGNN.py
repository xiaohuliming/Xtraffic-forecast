...
class IGSTGNN(BaseModel):
    ...
    def __init__(self, model_args, **args):
        ...
        data_dir, _, _ = get_dataset_info(dataset)
    ...
    def _prepare_inputs(self, history_data):
        ...
        time_in_day_feat = self.T_i_D_emb[(history_data[:, :, :, num_feat] * self._model_args['tpd']).long()]
        day_in_week_feat = self.D_i_W_emb[(history_data[:, :, :, num_feat+1] * 7).long()]
...
class IncidentsIcsfModule(nn.Module):
    ...
    def apply_incident_icsf_fusion(self, history_data, incident_data, sensor_data=None, incident_tod_feat=None, incident_day_feat=None):
        ...
        if sensor_embed is not None:
            incident_effect_init = torch.cat([
