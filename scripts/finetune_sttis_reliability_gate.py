#!/usr/bin/env python3
"""Fine-tune a branch-reliability adapter on a proposal-aware ST-TIS gate."""
...
from train_dual_branch_sttis_gate import DualBranchSTTISReliabilityGate
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/impact_guided_next_stage/dual_branch_sttis_reliability_gate_seed_23"),
    )
    parser.add_argument("--epochs", type=int, default=3)
    parser.add_argument("--batch-size", type=int, default=192)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--weight-decay", type=float, default=0.0)
    parser.add_argument("--reliability-loss-weight", type=float, default=0.03)
    ...
    parser.add_argument("--train-reliability-scale", type=float, default=0.75)
...
def freeze_except_reliability(model: nn.Module) -> int:
    trainable = 0
    for name, param in model.named_parameters():
        param.requires_grad = name.startswith("reliability_head.") or name.startswith("base_gate_logit_norm.")
        if param.requires_grad:
            trainable += param.numel()
    if trainable == 0:
        raise RuntimeError("no reliability parameters were marked trainable")
    return trainable
...
def compute_reliability_loss(
    ...
    target = torch.sigmoid(error_diff / max(args.preference_temperature, 1e-6))
    reliability_raw = nn.functional.binary_cross_entropy_with_logits(
        details["reliability_logits"],
        target,
        reduction="none",
    )
    if args.preference_margin > 0:
        pref_weight = (error_diff.abs() / args.preference_margin).clamp(0.0, 1.0)
    else:
        pref_weight = None
    reliability_loss = masked_mean(reliability_raw, y_mask, pref_weight)
    loss = residual_loss + args.reliability_loss_weight * reliability_loss
    with torch.no_grad():
        base_gate = details["base_gate"]
        gate = details["gate"]
        branch_gap = masked_mean(error_diff.abs(), y_mask)
    return loss, {
        "residual_loss": float(residual_loss.detach().cpu()),
        "reliability_loss": float(reliability_loss.detach().cpu()),
        "branch_gap": float(branch_gap.detach().cpu()),
...
def evaluate_loss(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            loss, parts = compute_reliability_loss(model, batch, args, device)
...
def save_training_plot(log_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("loss")
    ax.set_title("ST-TIS reliability adapter fine-tuning")
    ax.legend()
    fig.tight_layout()
...
def sweep_scale_beta(
    ...
    original_scale = float(model.reliability_scale)
    rows = []
    try:
        for scale in scales:
            model.reliability_scale = float(scale)
            metrics_by_beta = forecast_metrics_for_loader(model, loader, betas, device)
            rows.extend({"reliability_scale": scale, "residual_beta": beta, **values} for beta, values in metrics_by_beta.items())
    finally:
        model.reliability_scale = original_scale
...
def metrics_at(
    ...
    old_scale = float(model.reliability_scale)
    model.reliability_scale = float(reliability_scale)
    try:
        return forecast_metrics_for_loader(model, loader, [residual_beta], device)[residual_beta]
    finally:
        model.reliability_scale = old_scale
...
def write_summary(
    ...
    lines = [
        ...
        f"- reliability-gate all candidates robust MAE: `{test['all_candidates_model_robust_mae']:.4f}`",
        f"- reliability-gate affected candidates robust MAE: `{test['affected_candidates_model_robust_mae']:.4f}`",
        f"- reliability_scale: {reliability_scale:.2f}",
        ...
        f"- lr: {args.lr}",
        f"- reliability_loss_weight: {args.reliability_loss_weight}",
        f"- preference_temperature: {args.preference_temperature}",
        f"- preference_margin: {args.preference_margin}",
        f"- trainable reliability parameters: {trainable_params}",
...
def main() -> None:
    ...
    model = build_model(ckpt, cache_path, args.proposal_feature_count, args.train_reliability_scale, device)
    trainable_params = freeze_except_reliability(model)
    print(f"trainable reliability parameters: {trainable_params}", flush=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss, _parts = compute_reliability_loss(model, batch, loss_args, device)
            loss.backward()
    ...
    scales = [round(x, 2) for x in np.arange(0.0, 1.76, 0.25)]
    beta_candidates = [round(x, 2) for x in np.arange(0.85, 1.21, 0.05)]
    val_sweep = sweep_scale_beta(model, val_loader, scales, beta_candidates, device)
    val_sweep.to_csv(output_dir / "reliability_scale_beta_sweep.csv", index=False)
    best_row = val_sweep.loc[val_sweep["all_candidates_model_robust_mae"].idxmin()]
    reliability_scale = float(best_row["reliability_scale"])
    residual_beta = float(best_row["residual_beta"])
    model.reliability_scale = reliability_scale

    metrics: dict[str, dict[str, float]] = {}
    for split, idx in eval_indices.items():
        loader = make_loader(cache_path, idx, stats, args.batch_size, shuffle=False)
        metrics[split] = metrics_at(model, loader, reliability_scale, residual_beta, device)
    ...
    for code in sorted(np.unique(region_code_arr[eval_indices["test"]]).tolist()):
        ...
        row.update(metrics_at(model, loader, reliability_scale, residual_beta, device))
    ...
    ckpt_args["model_class"] = "DualBranchSTTISReliabilityGate"
    ckpt_args["training_variant"] = "reliability_gate_finetune"
    ckpt_args["reliability_scale"] = reliability_scale
    ...
    config["model_class"] = "DualBranchSTTISReliabilityGate"
    config["training_variant"] = "reliability_gate_finetune"
    config["device"] = str(device)
    config["cache_path"] = str(cache_path)
    config["reliability_scale"] = reliability_scale
    config["residual_beta"] = residual_beta
    with (output_dir / "config.json").open("w", encoding="utf-8") as f:
        json.dump(config, f, indent=2, ensure_ascii=False)
    write_summary(output_dir, args, source_metrics, metrics, split_counts, reliability_scale, residual_beta, trainable_params, log_df)
